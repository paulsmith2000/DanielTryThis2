...
// LoveLights based on ColorWavesWithPalettes
...
void setup() {
  ...
  delay(500);
  CRGB colA, colB;
  hsv2rgb_rainbow(CHSV(15, 230, 120), colA);
  hsv2rgb_rainbow(CHSV(95, 255, 120), colB);
  ...
  strip.addSegment(segment[0]);
  strip.addSegment(segment[1]);
  strip.addSegment(segment[2]);
  strip.addSegment(segment[3]);
  strip.addSegment(segment[4]);
}
...
{
  EVERY_N_SECONDS(6) {
    segment[1]->toggleOnOff();
    segment[2]->toggleOnOff();
    segment[3]->toggleOnOff();
  }
  strip.loop();
  FastLED.show();
}
...
// palette parameters = position in gradient to start, R value, G value, B value.
...
// Gradient palette "BlacK_Red_Yellow_zigzig_gp", originally from
// http://soliton.vm.bytemark.co.uk/pub/cpt-city/nd/basic/tn/BlacK_Red_Yellow_zigzig.png.index.html
// converted for FastLED with gammas (2.6, 2.2, 2.5)
// Size: 196 bytes of program space.

DEFINE_GRADIENT_PALETTE(BlacK_Red_Yellow_zigzig_gp ) {
  0, 255, 255,  0,
  5, 255, 55,  0,
  10, 255,  0,  0,
  15,  42,  0,  0,
